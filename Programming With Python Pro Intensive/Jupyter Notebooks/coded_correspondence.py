...
alphabet = "abcdefghijklmnopqrstuvwxyz"
punctuation = ".,?'! @#$$%^&*()+-_`;:\"/\\"
message = "i was able to decode the message"
translated_message = ""
...
def coder(message, offset):
    ...
    for letter in message:
        if not letter in punctuation:
            letter_value = alphabet.find(letter)
            encoded_message += alphabet[(letter_value - offset) % 26]
        else:
            encoded_message += letter
            
    return encoded_message

print(coder("This is a test message.", 10))
...
encoded_message = "vhfinmxkl atox kxgwxkxw tee hy maxlx hew vbiaxkl tl hulhexmx. px'ee atox mh kxteer lmxi ni hnk ztfx by px ptgm mh dxxi hnk fxlltzxl ltyx."

for i in range(1,25):
    print("offset: {0}".format(i))
    print("\t {0} \n".format(decoder(encoded_message, i)))
...
encoded_message = "thanks for all the help Vishal."
keyword = "buddies"
...
print(vigenere_encoder(encoded_message,keyword))
